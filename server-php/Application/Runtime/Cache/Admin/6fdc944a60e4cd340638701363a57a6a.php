                ...
                <nav class="navbar navbar-default">
                  ...
                  <div class="container-fluid">
                    ...
                    <div class="form-inline navbar-form navbar-left">
                      ...
                      <div class="form-group">
                        <button type="button" class="btn btn-success btn-sm template" title="学生档案信息模版">
                          <i class="fa fa-download"></i> 下载模板
                        </button>
                      </div>
                      <div class="form-group">
                        <button type="button" class="btn btn-success btn-sm import" title="学生档案信息批量导入">
        ...
        <div class="col-md-12">
          <div class="nav-tabs-custom">
            ...
            <div class="tab-content js_tab_content2">
              <div class="tab-pane">
                <form class="form-horizontal" action="" method="post" onsubmit="return false" id="stuinfoFormWindow">
                  ...
                  <div style="text-align:center;">
                    <button type="button" class="btn btn-success btn-sm js_stuInfoSave">
                      <span class="glyphicon glyphicon-save"></span> 保存
                    </button>
                  </div>
                </form>
...
<div id="js_tianjia_cont">
  <div class="row getLastRow js_getLastRow">
    <form class="form-horizontal" action="" method="post" onsubmit="return false" id="parentInfoFormWindow1">
      <input class="primaryKey js_stu_id" type="hidden" name="stu_id" />
      <input class="primaryKey" type="hidden" name="sp_id" id="sp_id" />
      <div class="col-md-12">
        <div class="row">
          <div class="col-md-6">
            <div class="form-group">
        ...
        <div class="row">
          <div class="col-md-6">
            <div class="form-group">
              ...
              <div class="col-md-7">
                <input type="text" placeholder="请用手机号设置登录账号" id="parent_phone" name="parent_phone" class="form-control" />
              </div>
            </div>
          </div>
          <div class="col-md-6">
      ...
      <div style="text-align:center;">
        <button type="button" class="btn btn-success btn-sm js_parentSave">
          <span class="glyphicon glyphicon-save"></span> 保存
        </button>
        <button type="button" class="btn btn-danger btn-sm js_parentDel">
...
<div class="row getLastRow js_getLastRow">
  <form class="form-horizontal" action="" method="post" onsubmit="return false" id="parentInfoFormWindow{{index}}">
    <input type="hidden" class="js_stu_id" value="{{stu_id}}" name="stu_id" />
  	...
  	<div class="col-md-12">
        ...
        <div class="col-md-6">
          <div class="form-group">
            ...
            <div class="col-md-7">
              <input type="text" placeholder="请用手机号设置登录账号" id="parent_phone" value="{{parent_phone}}" name="parent_phone" class="form-control" disabled="disabled" />
            </div>
          </div>
        </div>
  	    <div class="col-md-6">
...
if (user_type == '4') {
  $('#tab1 .navbar, #remove, th[data-checkbox="true"], th[data-formatter="studentFormatter"]').remove();
} else {
...
$(function() {
  ...
  $('.js_upgradeBtn').click(function() {
    ...
    if (bool) {
      var url = g_config.host + '/index.php/Admin/School/StudentInfo/Upgrade';
  ...
  function fnparentInfo(index) {
    html += '<div class="row getLastRow js_getLastRow">' +
              '<form class="form-horizontal" action="" method="post" onsubmit="return false" id="parentInfoFormWindow' + index + '">' +
                '<input class="primaryKeyn js_stu_id" type="hidden" name="stu_id" />' +
                '<input class="primaryKey" type="hidden" name="sp_id" id="sp_id" />' +
                '<div class="col-md-12">' +
                  '<div class="row">' +
                    '<div class="col-md-6">' +
                ...
                '</div>' +
                '<div style="text-align:center;">' +
                  '<button type="button" class="btn btn-success btn-sm js_parentSave" style="margin-right: 4px;">' +
                    '<span class="glyphicon glyphicon-save"></span> 保存' +
                  '</button>' +
                  '<button type="button" class="btn btn-danger btn-sm js_parentDel">' +
                    '<span class="glyphicon glyphicon-remove"></span> 删除' +
  ...
  window.studentoperateEvents = {
    ...
    'click .edit': function (e, value, row, index) {
      ...
      var url = g_config.host + '/index.php/Admin/School/StudentInfo/get';
      $('#stu_id').val(row.stu_id);
      if (row.imei_id == '') {
        $('#imei_id').attr('disabled', false);
      } else {
        $('#imei_id').attr('disabled', true);
      };
...
function ajaxRequest(params) {
  ...
  searchForm();
  // 需要的数据
  var url = "/index.php/Admin/School/StudentInfo/query?" + $("#searchForm").serialize();
...
// 保存学生信息
$(document).delegate('.js_stuInfoSave', 'click', function() {
  var aList = $(this).parents('#stuinfoFormWindow').find('.form-group');
  var bool  = true;
  ...
  // 保存提交
  if (bool) {
    var url              = g_config.host + '/index.php/Admin/School/StudentInfo/edit';
    var stuInfo_stuId    = $(this).parents('.js_tab_content2').find('#stu_id');
    var parentInfo_stuId = $(this).parents('.js_tab_content2').find('.js_stu_id');
    fpost(url,'stuinfoFormWindow',function(data) {
      console.log(data)
      if( data.status == 1 ) {
        dialog.success(data.data.info,function() {
          stuInfo_stuId.val(data.data.stu_id);
          parentInfo_stuId.val(data.data.stu_id);
          layer.closeAll('dialog');
          search();
          $('.js_nav_tab2 li:eq(1)').addClass('active').siblings('li').removeClass('active');
          $('.js_tab_content2 .tab-pane:eq(1)').addClass('active').show().siblings('.tab-pane').removeClass('active').hide();
        });
      } else {
        dialog.error(data.data.info);
      }
    });
  }
});
...
// 保存监护人信息
$(document).delegate('.js_parentSave', 'click', function() {
  var aList   = $(this).parents('.js_getLastRow').find('.form-group');
  var bool    = true;
  ...
  // 保存提交
  if (bool) {
    var parentUrl    = g_config.host + '/index.php/Admin/School/StudentInfo/partentEdit';
    var parentFromId = $(this).parents('.js_getLastRow').find('.form-horizontal').attr('id');
    var sp_id        = $(this).parents('.js_getLastRow').find('#sp_id');
    var parent_phone = $(this).parents('.js_getLastRow').find('#parent_phone');
    $(this).parents('.js_getLastRow').find('.js_stu_id').val($('#stu_id').val());
    fpost(parentUrl,parentFromId,function(data) {
      if( data.status == 1 ) {
        dialog.success(data.data.info,function() {
          sp_id.val(data.data.sp_id);
          parent_phone.attr('disabled', true);
          layer.closeAll('dialog');
          search();
        });
      } else {
...
// 删除监护人
$(document).delegate('.js_parentDel', 'click', function() {
  var url          = g_config.host + '/index.php/Admin/School/StudentInfo/partentDel';
  var parentDiv    = $(this).parents('.js_getLastRow');
  var parentFromId = parentDiv.find('.form-horizontal').attr('id');
  var sp_id        = parentDiv.find('#sp_id').val();
  parentDiv.find('.js_stu_id').val($('#stu_id').val());
  if (sp_id != '') {
    fpost(url,parentFromId,function(data) {
      if( data.status == 1 ) {
...
// 删除
function del() {
  var url    = g_config.host + "/index.php/Admin/School/StudentInfo/del";
  var ids    = getIdSelections($('.js-custom-table-data'),'stu_id');
  var strIds = ids.join(',');
  var data   = {};
  data.id    = strIds;
...
$('.template').click(function() {
  window.open('/Public/template/学生信息模板(内测版).xls');
})
// 下载设备批量绑定模板
...
$('.import').click(function() {
  if ($(".c_id_hide").val() == '') {
    ...
    var url  = '/index.php/Admin/School/StudentInfo/import?s_id='+s_id+'&a_id='+a_id+'&g_id='+g_id+'&c_id='+c_id;
...
function ajaxRequest2(params, id) {
  ...
  var url = "/index.php/Admin/School/StudentInfo/inguiry_unit?" + $("#searchForm2").serialize();
  queryList(url,params);
}
...
function search2() {
  // form查询域赋值
  searchForm2();
  var url = "/index.php/Admin/School/StudentInfo/inguiry_unit?type=1&" + $("#searchForm2").serialize();

...
class DeptModel extends BaseModel{
	...
	protected $_validate =array(
		...
		// array('name','','部门名称已存在',1,'unique',1),
	    array('name', 'checklength', '名称长度必须在2-16位之间！', 0, 'callback', 3, array(2, 16)),
	    array('d_id,a_id,name','checkUnique','部门名称不能重复',self::MUST_VALIDATE,'callback',self::MODEL_BOTH), // 自定义函数验证规则 	    // array('memo','require','备注不能为空'),
	);
    ...
    function checklength($str, $min, $max) {
     ...
     public function checkUnique($data){
        $areaInfo = self::getWhereInfoEx('d_id', array('a_id'=>$data['a_id'],'name'=>$data['name']));
        // var_dump($areaInfo);die;
        // var_dump($data['d_id']);
        if($areaInfo&&$areaInfo['d_id'] != $data['d_id'])
            return false;
        else
            return true;
    }

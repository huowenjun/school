var Swiper = function(selector, params) {
    ...
    var defaults = {
        eventTarget: 'wrapper',
        mode: 'horizontal',  //horizontal/vertical 选择水平或垂直模式
        touchRatio: 1,
        speed: 500,  //速度，设置动画持续时间ms
        freeMode: false,  //开启自由模式
        freeModeFluid: false,  //开启'fluid'自由模式
        momentumRatio: 1,
        momentumBounce: false,  // true/false 是否禁用自由模式下的（free mode）动量弹性
        momentumBounceRatio: 30,  //值越大产生的动量弹性（momentum bounce）效果越明显
        slidesPerView: 1,
        slidesPerGroup: 1,
        ...
        shortSwipes: true,
        longSwipesRatio: 0.5,
        moveStartThreshold: false,  //滑动的临界值，临界值单位为px，如果触屏距离小于该值滑块不会运动
        onlyExternal: false,
        ...
        noSwiping: false,  //html元素使用的类名，当noSwiping参数设置为true时，用于禁止滑动
        ...
        autoplay: 3000,  // 自动播放 是/否 或者过度延迟时间/ms
        autoplayDisableOnInteraction: true,  //false  用户操作之后每次都会禁止启动autoplay(自动播放)
    ...
    function preventClick(e) {
        if (!_this.allowLinks) {
            if (e.preventDefault) e.preventDefault();
            else e.returnValue = false; if (params.preventLinksPropagation && 'stopPropagation' in e) {
                e.stopPropagation();
            }
            return false;
        }

        ...
        if ($worker_id == 23) {
            swoole_timer_tick(2000, function () {
                ...
                foreach ($this->server->connections as $fd) {
                    $imei = $redis->get('FD:' . $fd);
                    //处理平台命令
                    $taskArr = $this->get_task($imei);
                    foreach ($taskArr as $orders) {
                        $this->server->send($fd, $orders);
                    }
                    unset($taskArr);
                }
            });
...
//        echo $fd . "----connected\n";
    ...
    public function onSwooleReceive($serv, $fd, $reactorId, $data)
    {
        ...
        if (strpos($data, '222519') !== false) {
            file_put_contents('222519.txt', date('Y-m-d H:i:s') . $data . PHP_EOL, FILE_APPEND);
        }
        try {
            //判断设备类型
            $command = array(0);
            ...
            if ($command[0]) {
                ...
                foreach ($command[0] as $key => $command_str) {
                    ...
                    $redis = self::redis();
                    //校验FD
                    $redis->set('FD:' . $fd, $command_arr[1],3600);
                    //逻辑处理部分
...
//                            $serv->send($fd, $reply);
                        ...
                        if ($is_pp === true) { //自己可交友
                            ...
                            $redis->expire($ppkey, 20);
                            foreach ($imeis as $imei) {
                                if ($imei != $command_arr[1]) {//好友imei
                                    $is_pp_check = $this->checkPP($imei);//检测好友是否可以交友
                                    if (($is_pp_check != $command_arr[1]) && ($is_pp_check !== true)) {//好友存在好友
                                        ...
                                        $db->exec("update sch_student set f_id = '{$imei}' where imei_id = '{$command_arr[1]}'");
                                    }
                                }
                            }
                    ...
                    if (isset($content[3])) {
                        if (($comment_type == 'AL') || ($comment_type == 'UD') || ($comment_type == 'UD2') || ($comment_type == 'WT')) {
                            ...
                            MyPDO::insert($db,'sch_trail',$db_data);
                        }
                    }
                }
            } else {
...
//        echo $fd . "----swooleClosed\n";
    ...
    private function save_amr($amr_content, $imei)
    {
        ...
        include_once 'Easemob.php';
        $easemobM = Easemob::getInstance();
        $easemobM->sendMseeage(
            [
                'file'=>$file_name,
                'type'=>3,
                'imei'=>$imei
...
$config = ['server' =>
    ['worker_num' => 4,
        ...
        'log_file' => '/phpstudy/www/swoole/error2-2.log',

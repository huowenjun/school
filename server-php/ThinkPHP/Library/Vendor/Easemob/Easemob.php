...
class message
{
    ...
    protected $orgName = '1152171201178499';
    protected $appName = 'shuhai-anfang';
    protected $clientID = 'YXA6ecX9ANZsEeepVys2kFMPEw';
    protected $clientSecret = 'YXA69MLdG6oM-FXGLvStju-_qZ4ja3U';
    /*
     * 发送消息
     * $data array(
     *  file 文字 图片 语音
    ...
    protected function processingData($data){
        if($data['type']==2||$data['type']==3){//图片，语音先上传服务器
            $fileArr = $this->fileUpload($data['file']);
            if($data['type']==2){
                $jsonData['msg']=[
                    'type'=>'img',
                    'url'=>$fileArr['uuid'],

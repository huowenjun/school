...
abstract class Driver {
    ...
    // 数据库表达式
    protected $exp = array('eq'=>'=','neq'=>'<>','gt'=>'>','egt'=>'>=','lt'=>'<','elt'=>'<=','notlike'=>'NOT LIKE','like'=>'LIKE','in'=>'IN','notin'=>'NOT IN','not in'=>'NOT IN','between'=>'BETWEEN','not between'=>'NOT BETWEEN','notbetween'=>'NOT BETWEEN','find'=>'FIND_IN_SET');
    // 查询表达式
    ...
    protected function parseWhereItem($key,$val) {
        ...
        if(is_array($val)) {
            if(is_string($val[0])) {
                ...
                }elseif(preg_match('/^(find)$/',$exp)){ // find_in_set
                    $whereStr .= $this->exp[$exp].' (\''. intval($val[1]) . '\',' . $key . ')';
                }else{

...
class Page{
    ...
    public function show() {
        ...
        if($this->totalPages > 1){
            //跳转页面
            $page_str .= '<li>跳转至&nbsp;<input type="text" id="txt_page" class="input-mini focused" placeholder="页数" style="margin-bottom:0;">&nbsp;页&nbsp;<input type="button" title="跳转" value="跳转" class="btn" onclick="javascript:pageJump();"></li>';
            $page_str .= '<script>
                function pageJump(){
                    var ye = $("#txt_page").val();
                    ye = parseInt(ye);     
        		    if(ye < 1 || ye > '.$this->totalPages.' || isNaN(ye) ){
        			     return;
        		    };
                    var href = "'.$this->url.'";
        		    href = href.replace("%5BPAGE%5D",ye);
        		    window.location.href=href;
                };
                </script>';
        }
        return '<ul class="pagination pagination-left">'.$page_str.'</ul>';
    }
}
